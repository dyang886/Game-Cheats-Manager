...
public:
    ...
    // Constants for process name and pointer chain
    static inline const wchar_t *PROCESS_NAME = L"Headbangers.exe"; // x64
    static inline const wchar_t *MODULE_NAME = L"GameAssembly.dll";
    std::vector<unsigned int> OFFSETS_bread_value = {0x036E2A88, 0xA0, 0xE60, 0x48, 0x18, 0x10, 0x30, 0x20};
    ...
    // Check if the game process is running and open a handle
    bool isProcessRunning()
    {
        procId = getProcId(PROCESS_NAME);
        if (procId == 0)
        {
            std::wcerr << L"Process not found: " << PROCESS_NAME << std::endl;
            return false;
        }
    ...
    void setBread(int currency)
    {
        uintptr_t finalAddress = resolveModuleDynamicAddress(MODULE_NAME, OFFSETS_bread_value);
        if (finalAddress == 0)
        {
            std::cerr << "Failed to resolve currency value address." << std::endl;
            return;
        }

        if (!writeMemory<int>(finalAddress, currency))
        {
            std::cerr << "Failed to write currency value." << std::endl;
        }
    }
...
private:
    ...
    uintptr_t getModuleBaseAddress(DWORD procId, const wchar_t *modName)
    {
        ...
        {
            std::cerr << "Failed to create snapshot when getting module base address. Error: " << GetLastError() << std::endl;
        }
        CloseHandle(hSnap);
        return modBaseAddr;
    }

    uintptr_t resolveModuleDynamicAddress(const wchar_t *module_name, const std::vector<unsigned int> &offsets)
    {
        uintptr_t modBaseAddr = getModuleBaseAddress(procId, module_name);
        if (modBaseAddr == 0)
        {
            std::wcerr << L"Failed to find module base address for " << module_name << std::endl;
            return 0;
        }

        uintptr_t baseAddress = modBaseAddr + offsets[0];
        std::vector<unsigned int> pointerOffsets(offsets.begin() + 1, offsets.end());
        return resolveDynamicAddress(hProcess, baseAddress, pointerOffsets);
    }

    uintptr_t resolveDynamicAddress(HANDLE hProc, uintptr_t ptr, const std::vector<unsigned int> &offsets)
    {
        uintptr_t addr = ptr;
        for (unsigned int offset : offsets)
        {
            ...
            {
                std::cerr << "Failed to dereference pointer at address: 0x" << std::hex << addr << std::endl;
                return 0;
            }
            addr += offset;
        }
    ...
    template <typename T>
    bool writeMemory(uintptr_t resolvedAddress, T value)
    {
        if (!WriteProcessMemory(hProcess, (BYTE *)resolvedAddress, &value, sizeof(T), nullptr))
        {
