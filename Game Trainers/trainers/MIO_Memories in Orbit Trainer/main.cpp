// Native memory trainer: patches mio.exe directly (byte patches, code-cave hook,
// static pointer writes) via TrainerBase. No injection / IL2CPP.
...
// Version info
static constexpr const char *TRAINER_NAME = "MIO: Memories in Orbit Trainer";
static constexpr const char *GAME_VERSION = "r21756";
...
void apply_callback(Fl_Widget *widget, void *data)
{
    ...
    try
    {
        ...
        if (optionName == "SetNacreDroplets")
            status = trainer->setNacreDroplets(std::stoi(val));
        else if (optionName == "SetCrystallizedNacre")
            status = trainer->setCrystallizedNacre(std::stoi(val));
        else if (optionName == "SetOldCore")
            status = trainer->setOldCore(std::stoi(val));
    }
    catch (...)
    {
        status = false;
...
void toggle_callback(Fl_Widget *widget, void *data)
{
    ...
    try
    {
        if (optionName == "InfiniteHealth")
            status = enabled ? trainer->infiniteHealth() : trainer->disableNamedHook("InfiniteHealth");
        else if (optionName == "UnlimitedModifierAllocation")
            status = enabled ? trainer->unlimitedModifierAllocation() : trainer->disableNamedHook("UnlimitedModifierAllocation");
        else if (optionName == "InfiniteJumps")
            status = enabled ? trainer->infiniteJumps() : trainer->disableNamedHook("InfiniteJumps");
        else if (optionName == "InfiniteStamina")
            status = enabled ? trainer->infiniteStamina() : trainer->disableNamedPointerToggle("InfiniteStamina");
        else if (optionName == "MovementSpeedMultiplier")
        {
            if (enabled)
            {
                std::string mval = (toggleData->input && toggleData->input->value()[0]) ? toggleData->input->value() : "1";
                status = trainer->movementSpeedMultiplier(std::stof(mval));
            }
            else
            {
                bool d1 = trainer->disableNamedHook("MovementSpeedMultiplierH1");
                bool d2 = trainer->disableNamedHook("MovementSpeedMultiplierH2");
                status = d1 && d2;
            }
        }
    }
...
int main(int argc, char **argv)
{
    ...
    options_flex->margin(70, 20, 20, 20);
    options_flex->gap(8);

    Fl_Box *spacerTop = new Fl_Box(0, 0, 0, 0);

    place_toggle_widget(options_flex, &trainer, "InfiniteHealth", "Infinite Health");

    place_toggle_widget(options_flex, &trainer, "InfiniteJumps", "Infinite Jumps");

    place_toggle_widget(options_flex, &trainer, "InfiniteStamina", "Infinite Stamina");

    place_toggle_widget(options_flex, &trainer, "UnlimitedModifierAllocation", "Unlimited Modifier Allocation");

    place_toggle_widget(options_flex, &trainer, "MovementSpeedMultiplier", "Movement Speed Multiplier", nullptr, "2", "0", "100", FL_FLOAT_INPUT);

    place_apply_widget(options_flex, &trainer, "SetNacreDroplets", "Set Nacre Droplets", nullptr, "99999", "0", "999999999");

    place_apply_widget(options_flex, &trainer, "SetCrystallizedNacre", "Set Crystallized Nacre", nullptr, "99999", "0", "999999999");

    place_apply_widget(options_flex, &trainer, "SetOldCore", "Set Old Core", nullptr, "99", "0", "999999999");
