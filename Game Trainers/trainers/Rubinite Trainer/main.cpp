...
// Version info
static constexpr const char *TRAINER_NAME = "Rubinite Trainer";
static constexpr const char *GAME_VERSION = "0e4f340";
...
void toggle_callback(Fl_Widget *, void *data)
{
    ...
    try
    {
        ...
        if (optionName == "ToggleGodMode")
            status = trainer->toggleGodMode(isEnabled, message);
        else if (optionName == "ToggleInfiniteHealth")
            status = trainer->toggleInfiniteHealth(isEnabled, message);
        else if (optionName == "SetMoveSpeed")
            status = trainer->setMoveSpeed(isEnabled, std::stof(val), message);
        else if (optionName == "SetTalismanSlots")
            status = trainer->setTalismanSlots(isEnabled, std::stoi(val), message);
        else if (optionName == "SetDamageMultiplier")
            status = trainer->setDamageMultiplier(isEnabled, std::stof(val), message);
        else
            status = false;
    }
    catch (...)
    ...
    if (!status)
    {
        ...
        std::string msg = message.empty() ? t("Failed to activate / deactivate.") : t(message);
        if (message.empty())
        {
...
void apply_callback(Fl_Widget *widget, void *data)
{
    ...
    try
    {
        const std::string val = (input && input->value()[0]) ? input->value() : "0";

        if (optionName == "SetBloodDust")
            status = trainer->setBloodDust(std::stoi(val), message);
        else if (optionName == "SetImpureCrystal")
            status = trainer->setImpureCrystal(std::stoi(val), message);
        else
            status = false;
    }
    catch (...)
...
int main(int argc, char **argv)
{
    ...
    place_toggle_widget(options_flex, &trainer, "ToggleInfiniteHealth", "Infinite Health");

    place_toggle_widget(options_flex, &trainer, "SetMoveSpeed", "Move Speed", nullptr, "7", "0.1", "100", FL_FLOAT_INPUT);

    place_toggle_widget(options_flex, &trainer, "SetDamageMultiplier", "Damage Multiplier", nullptr, "5", "0", "10000", FL_FLOAT_INPUT);

    place_toggle_widget(options_flex, &trainer, "SetTalismanSlots", "Talisman Slots", nullptr, "10", "1", "10", FL_INT_INPUT);

    place_apply_widget(options_flex, &trainer, "SetBloodDust", "Set Blood Dust", nullptr, "99999", "0", "999999999");

    place_apply_widget(options_flex, &trainer, "SetImpureCrystal", "Set Impure Crystal", nullptr, "99", "0", "999999999");
