// Uses Mono injection through MonoBase and the embedded GCMInjection assembly.
...
// Version info
static constexpr const char *TRAINER_NAME = "Minishoot' Adventures Trainer";
static constexpr const char *GAME_VERSION = "1.05";
...
/// Apply callback - called by Fl_Button widgets via FLTKUtils.h
void apply_callback(Fl_Widget *widget, void *data)
{
    ...
    try
    {
        if (optionName == "SetStatsPoints")
            status = trainer->setStatsPoints(std::stoi(val));
        else if (optionName == "SetSuperCrystals")
            status = trainer->setSuperCrystals(std::stoi(val));
    }
    catch (...)
    {
        status = false;
...
/// Toggle callback - called by Fl_Check_Button widgets via FLTKUtils.h
void toggle_callback(Fl_Widget *widget, void *data)
{
    ...
    bool status = true;
    std::string val = (toggleData->input && toggleData->input->value()[0]) ? toggleData->input->value() : "0";

    try
    {
        if (optionName == "ToggleGodMode")
            status = trainer->toggleGodMode(enabled);
        else if (optionName == "SetHealth")
            status = trainer->setHealth(enabled, std::stoi(val));
        else if (optionName == "ToggleInfiniteEnergy")
            status = trainer->toggleInfiniteEnergy(enabled);
        else if (optionName == "SetMovementSpeedMultiplier")
            status = trainer->setMovementSpeedMultiplier(enabled, std::stof(val));
        else if (optionName == "SetDamageMultiplier")
            status = trainer->setDamageMultiplier(enabled, std::stof(val));
        else if (optionName == "SetFireRateMultiplier")
            status = trainer->setFireRateMultiplier(enabled, std::stof(val));
        else if (optionName == "SetBulletSpeedMultiplier")
            status = trainer->setBulletSpeedMultiplier(enabled, std::stof(val));
        else if (optionName == "SetFireRangeMultiplier")
            status = trainer->setFireRangeMultiplier(enabled, std::stof(val));
        else if (optionName == "SetSpiritPowerMultiplier")
            status = trainer->setSpiritPowerMultiplier(enabled, std::stof(val));
        else if (optionName == "ToggleFloatOverVoidWater")
            status = trainer->toggleFloatOverVoidWater(enabled);
        else if (optionName == "ToggleNoPowerCooldown")
            status = trainer->toggleNoPowerCooldown(enabled);
    }
    catch (...)
    {
        status = false;
    ...
    if (toggleData->input)
        button->value() ? toggleData->input->readonly(1) : toggleData->input->readonly(0);
}
...
int main(int argc, char **argv)
{
    ...
    place_toggle_widget(options_flex, &trainer, "SetHealth", "Set Health", nullptr, "10", "0", "999999999");

    place_toggle_widget(options_flex, &trainer, "ToggleInfiniteEnergy", "Infinite Energy");

    place_toggle_widget(options_flex, &trainer, "SetMovementSpeedMultiplier", "Movement Speed Multiplier", nullptr, "5", "0", "999999999", FL_FLOAT_INPUT);

    place_toggle_widget(options_flex, &trainer, "SetDamageMultiplier", "Damage Multiplier", nullptr, "5", "0", "999999999", FL_FLOAT_INPUT);

    place_toggle_widget(options_flex, &trainer, "SetFireRateMultiplier", "Fire Rate Multiplier", nullptr, "5", "0", "999999999", FL_FLOAT_INPUT);

    place_toggle_widget(options_flex, &trainer, "SetBulletSpeedMultiplier", "Bullet Speed Multiplier", nullptr, "5", "0", "999999999", FL_FLOAT_INPUT);

    place_toggle_widget(options_flex, &trainer, "SetFireRangeMultiplier", "Fire Range Multiplier", nullptr, "5", "0", "999999999", FL_FLOAT_INPUT);

    place_toggle_widget(options_flex, &trainer, "SetSpiritPowerMultiplier", "Spirit Fire Rate and Speed Multiplier", nullptr, "2", "0", "999", FL_FLOAT_INPUT);

    place_toggle_widget(options_flex, &trainer, "ToggleFloatOverVoidWater", "Float Over Void/Water");

    place_toggle_widget(options_flex, &trainer, "ToggleNoPowerCooldown", "No Power Cooldown");

    place_apply_widget(options_flex, &trainer, "SetStatsPoints", "Set Stats Points", nullptr, "999", "0", "999999999");

    place_apply_widget(options_flex, &trainer, "SetSuperCrystals", "Set Super Crystals", nullptr, "9999", "0", "999999999");
