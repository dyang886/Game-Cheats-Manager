// Uses CDP (Chrome DevTools Protocol) to inject JavaScript into the game's Electron window.
...
// Version info
static constexpr const char *TRAINER_NAME = "Zao Meng Xi You: Wu Shuang Trainer";
static constexpr const char *GAME_VERSION = "1.007";
...
/// Apply callback — required by FLTKUtils.h forward declaration but unused
...
void toggle_callback(Fl_Widget *widget, void *data)
{
    ...
    if (optionName == "ToggleInfiniteHp")
        status = trainer->toggleInfiniteHp(enabled);
    else if (optionName == "ToggleInfiniteMp")
        status = trainer->toggleInfiniteMp(enabled);
    else if (optionName == "ToggleInvincibilityMode")
        status = trainer->toggleInvincibilityMode(enabled);
    else if (optionName == "ToggleDamageReceivedMultiplier")
        status = trainer->toggleDamageReceivedMultiplier(enabled, val);
    else if (optionName == "ToggleAttackDamageMultiplier")
        status = trainer->toggleAttackDamageMultiplier(enabled, val);
...
{
    Trainer *trainer = static_cast<Trainer *>(data);
    if (!trainer->launchGame())
        trainer_alert(t("Failed to launch game."));
}
...
int main(int argc, char **argv)
{
    ...
    // --- Launch ---
    {
        Fl_Flex *launch_flex = new Fl_Flex(0, 0, 0, 0, Fl_Flex::HORIZONTAL);
        launch_flex->gap(5);

        Fl_Box *launch_spacer = new Fl_Box(0, 0, 0, 0);

        Fl_Button *launch_btn = new Fl_Button(0, 0, 0, 0);
        launch_flex->fixed(launch_btn, 120);
        tr(launch_btn, "Launch Game");
        launch_btn->callback(launch_callback, &trainer);

        launch_flex->end();
        options_flex->fixed(launch_flex, UI_OPTION_HEIGHT);
    }
    ...
    // --- Battle Cheats ---
    place_toggle_widget(options_flex, &trainer, "ToggleInfiniteHp", "Infinite HP");

    place_toggle_widget(options_flex, &trainer, "ToggleInfiniteMp", "Infinite MP");

    place_toggle_widget(options_flex, &trainer, "ToggleInvincibilityMode", "Invincibility Mode");

    place_toggle_widget(options_flex, &trainer, "ToggleDamageReceivedMultiplier", "Damage Received Multiplier", nullptr, "10", "0", "999999999");

    place_toggle_widget(options_flex, &trainer, "ToggleAttackDamageMultiplier", "Attack Damage Multiplier", nullptr, "10", "0", "999999999");
